...
function displaySearchResults(query) {
    ...
    resultsContainer.innerHTML = results.length > 0
        ? results.map(ingredient => `
            <li data-name="${ingredient.name}" data-price="${ingredient.price}">
                ${ingredient.name} - ${ingredient.price} CFA
            </li>
        `).join('')
        : '<li>Aucun résultat trouvé.</li>';

    // Ajouter un écouteur d'événements sur les résultats de recherche
    resultsContainer.querySelectorAll('li').forEach(item => {
        item.addEventListener('click', () => {
            addIngredientToSelection(item.dataset.name, parseInt(item.dataset.price));
        });
    });
}

// Fonction pour ajouter un ingrédient à la sélection
function addIngredientToSelection(name, price) {
    // Vérifier si l'ingrédient est déjà sélectionné
    const isAlreadySelected = Array.from(selectedIngredients).some(
    ...
    // Créer une nouvelle carte d'ingrédient
    const ingredientCard = document.createElement('div');
    ingredientCard.classList.add('ingredient-card', 'selected');
    ingredientCard.dataset.price = price;
    ingredientCard.textContent = `${name} (+${price} CFA)`;

    // Ajouter l'ingrédient à la grille des ingrédients sélectionnés
    const ingredientGrid = document.getElementById('ingredientGrid');
    ingredientGrid.appendChild(ingredientCard);

    // Mettre à jour la sélection et le prix total
    selectedIngredients.add(ingredientCard);
    totalPrice += price;
    updatePriceDisplay();
    checkValidation();

    // Ajouter un écouteur d'événements pour supprimer l'ingrédient
    ingredientCard.addEventListener('click', () => {
        ingredientCard.remove();
        selectedIngredients.delete(ingredientCard);
        totalPrice -= price;
        updatePriceDisplay();
        checkValidation();
    });
...
// Fonction pour vérifier la validation (au moins 4 ingrédients)
function checkValidation() {
    const validationMsg = document.getElementById('validationMsg');
    validationMsg.style.display = selectedIngredients.size < 4 ? 'block' : 'none';
}
